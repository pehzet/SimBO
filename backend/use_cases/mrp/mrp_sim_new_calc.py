...
class MRPSimulation():
    ...
    def run_simulation(self):
        ...
        self.write_costs_csv(int(sum(self.costs)), self.storage_costs, _all_penalty_costs)
        return {"costs" : int(sum(self.costs)), "service_level" : sum(self.sl)/len(self.sl)}

    def write_costs_csv(self, all_costs, storage_costs, penalty_costs):
        with open("costs.csv", "a",encoding='utf-8',newline='') as f:
            w = csv.writer(f, delimiter=";")
            w.writerow([datetime.datetime.now().isoformat(), int(all_costs), int(storage_costs), int(penalty_costs)])

...
class MRPSimulation():
    ...
    def run_simulation(self, sim_time=200):
        ...
        for period in range(1, sim_time + 20):
            ...
            for o in orders_in_period:
                ...
                _fulfill_quant = 0
                # Case Fulfillment = 0
                if m.quantity_in_stock <= 0 and not o.backorder:
    ...
    def write_costs_csv(self, all_costs, storage_costs, sl):
        with open("costs.csv", "a",encoding='utf-8',newline='') as f:
            w = csv.writer(f, delimiter=";")
            w.writerow([datetime.datetime.now().isoformat(), int(all_costs), int(storage_costs), float(sl)] )
    ...
    def save_object_instances_csv(self):
        ...
        with open(file_präfix +"_materials.csv", 'w',encoding='utf-8',newline='') as f:
            header = list(self.materials[0].__dict__.keys()) + ["storage_costs"]
            w = csv.DictWriter(f, header)
            w.writeheader()

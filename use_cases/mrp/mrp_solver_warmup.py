...
class Material():
    ...
    def __init__(self,material_dict:dict, bom, initial_stock, safety_stock_param, safety_time_param, horizon, warmup) -> None:
        for k, v in material_dict.items():
            setattr(self, k, v)
            self.bom = bom
            self.greq = [0] * (horizon+ warmup +1)
            self.released_orders = [0] * (horizon + warmup +1)
...
class MRPSolver():
    def __init__(self, bom, materials, orders, stock, parameters, horizon=200, warmup=50) -> None:
        ...
        self.materials = [{m["id"] : self.init_material(m)} for m in materials]
    def init_material(self, m):
        ...
        return Material(m, bom, initial_stock, ss, st, self.horizon,self.warmup)
    ...
    def run(self):
        ...
        releases_listed_by_material = [m.values()[0].release_order() for m in self.materials]
        print(releases_listed_by_material)
